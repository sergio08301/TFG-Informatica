%Script para extraer la marca de agua de unas firmas determinadas y
%comproba que % contiene bits erroneos, cargar antes el .mat generado de
%conclusiongenerator.
%testDTW2firmasd d=derivatives of features

representarDTW=true;
representarFirmas=true;
aciertos = 0;
fallos = 0;
point=11;
...
directorioRobot='C:\firmasconclusiones\'; 
%listafirmantes=[0:144,200:274,300:374,400:434];
%listafirmantes=[0:144];                            %Para testeo
listafirmantes=[20:39]; 
errorReg=[];
for firma=1:5
    firmaRobot=leer_firma(data{firma,6});
    numero_firmante=data{firma,7};
    alpha=data{firma,5};
    watermark1=data{firma,2};
    watermark2=data{firma,3};
    watermarks = [watermark1, watermark2];
    firma_original=data{firma,1};
    
    watermarked_signal=trasladaFirma(firmaRobot,PCX,PCY);
    firma_original=trasladaFirma(firma_original,PCX,PCY);
    dct_original = dct2(firma_original(:,1:2));
    dct_watermarked = dct2(watermarked_signal(:,1:2));

    if representarFirmas
        figure %Compare non watermarked with watermark with specific alpha
        subplot(211)
        plot(watermarked_signal(:,1),watermarked_signal(:,2),'k-')
        title(sprintf('Representación de la firma robótica %d v%d con watermark de %d', numero_firmante, 6,alpha));
        grid on
        set(gca,'fontsize',16)
        
         %Compare non watermarked with watermark with specific alpha
        
        subplot(212)
        plot(firma_original(:,1),firma_original(:,2),'k-')
        title(sprintf('Representación de la firma humana original %d v%d', numero_firmante, 6));
        grid on
        set(gca,'fontsize',16)
    end

    if representarDTW
        [distancia,i1,i2] = dtw(watermarked_signal(:, 1:2)', firma_original(:, 1:2)');
         figure
         plot(i1,i2,'.-',[i1(1) i1(end)],[i2(1) i2(end)])
         title(sprintf('DTW de firma robótica %d v%d con watermark de alpha %d con firma humana original %d v%d', numero_firmante, 6, alpha,numero_firmante, 6));
    end
    % Realizar DTW en las dos primeras columnas para extraer la marca de agua
    [distancia, i1, i2] = dtw(dct_watermarked(:, 1:2)', dct_original(:, 1:2)');
    aligned_dct_watermarked = dct_watermarked(i1, 1:2);
    aligned_dct_original = dct_original(i2, 1:2);
    extracted_watermark = round((aligned_dct_watermarked - aligned_dct_original) / alpha);
    
    % Realizar DTW entre extracted_watermark y watermarks
    [distancia_wm, i1_wm, i2_wm] = dtw(extracted_watermark', watermarks');
    
    % Crear matrices alineadas basadas en los índices de DTW
    aligned_extracted_watermark = extracted_watermark(i1_wm, :);
    aligned_watermarks = watermarks(i2_wm, :);
    
    % Ejecutar la operación XOR y luego SUM en las matrices alineadas
    errors = sum(xor(aligned_extracted_watermark, aligned_watermarks), 'all');
    ...
    errorReg{firma}=100*errors/numel(extracted_watermark)';

end
  
% Paso 1: Extraer valores no vacíos
all_errors = [];
for k = 1:length(errorReg)
    if ~isempty(errorReg{k})
        all_errors = [all_errors, errorReg{k}];
    end
end

% Convertir all_errors a un vector si es una matriz
all_errors = all_errors(:);

% Paso 2: Calcular estadísticas
mean_errors = mean(all_errors);
mode_errors = mode(all_errors);
median_errors = median(all_errors);
std_errors = std(all_errors);
min_errors = min(all_errors);
max_errors = max(all_errors);

% Paso 3: Imprimir los resultados
fprintf('Media de errores: %.2f\n', mean_errors);
fprintf('Valores más comunes de errores: %.2f\n', mode_errors);
fprintf('Mediana de errores: %.2f\n', median_errors);
fprintf('Desviación estándar de errores: %.2f\n', std_errors);
fprintf('Mínimo error: %.2f\n', min_errors);
fprintf('Máximo error: %.2f\n', max_errors);
