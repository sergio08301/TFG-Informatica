...
watermark_length=fix(length(vect)/2);
signal_length=2*watermark_length;
is_odd_length = mod(length(vect(:,1)), 2) ~= 0;
...
index=1;
for alpha=alphaset
     % Embedding the watermark in the LL subband
    ...
    [LLwrx, HLwrx] = dwt(watermarked_signal1(1:signal_length), wname);
    [LLwry, HLwry] = dwt(watermarked_signal2(1:signal_length), wname);

    extracted_watermark1 = round((LLwrx - LLx) / alpha);
    extracted_watermark2 = round((LLwry - LLy) / alpha);

    
    MSE1D(index) = immse(vect(:,1:2),[watermarked_signal1,watermarked_signal2]);
    MAE1D(index) = mae(vect(:,1:2),[watermarked_signal1,watermarked_signal2]);
    disp(['DCT-1D MSE1D=',num2str(MSE1D(index)),' MAE1D=',num2str(MAE1D(index))])
    ...
    disp(['Alpha=',num2str(alpha),' erroneous bits=',num2str(errors),', % erroneous bits=',num2str(50*errors/numel(extracted_watermark1)),'%'])
    percentage_error(index)=50*errors/numel(extracted_watermark1);
    index=index+1;
end
figure(2)
clf
