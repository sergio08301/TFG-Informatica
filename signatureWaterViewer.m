% Inserción de watermark usando DCT. Este programa sirve principalmente para
% poder visualizar diversos casos de la marca de agua, y su objetivo es la 
% extraccion de capturas de pantalla para el proyecto.
...
vectZ=addZvalue( horzcat(vect(:,1), vect(:,2), vect(:,3)));
figure(8);
% Primer subplot
subplot(511)
plot(vect(:,1), vect(:,2), 'ko-')
title('signature user 414')
grid on
set(gca, 'fontsize', 16)
...
figure(1) %Compare non watermarked with watermark with specific alpha
subplot(211)
...
title(['Watermarked con alpha ' num2str(alpha) ' MCYT user=414']);
grid on
set(gca,'fontsize',16)
...
figure(5) %Show change in x and y in the watermarked version
clf
subplot(211)
...
index=1;        %Show that the %of error doesn't increase with the alpha
alphaset=[0.1:100:alpha];
...
for alpha=alphaset
    ...
    MSE2D= immse(vect(:,1:2),watermarked_signal);
    MAE2D= mae(vect(:,1:2),watermarked_signal);
...
for alpha=alphaset
    ...
    disp(['DCT-1D MSE1D=',num2str(MSE1D),' MAE1D=',num2str(MAE1D)])
    disp(['Error in watermark 1D MSE=',num2str(immse([extracted_watermark1,extracted_watermark2],[watermark1,watermark2])),' MAE=',num2str(mae([extracted_watermark1,extracted_watermark2],[watermark1,watermark2]))])
    %number of erroneous bits
    errors=sum(xor([extracted_watermark1,extracted_watermark2],[watermark1,watermark2]),'all');
    %disp(['Alpha=',num2str(alpha),' erroneous bits=',num2str(errors),', % erroneous bits=',num2str(100*errors/numel(extracted_watermark)),'%'])
    percentage_error(index)=100*errors/numel(extracted_watermark);
    index=index+1;
end
...
% Subplot 1
subplot(2, 1, 1) % Subplot en la primera posición
watermarked_signal=addWatermark(vect,100); 
[vectn]=featuresdd(watermarked_signal(:,1),watermarked_signal(:,2),vect(:,3));
plot(vectn(:,1),vectn(:,2),'k-')
title(['Watermarked con alpha ' num2str(100) ' MCYT user=414']);
grid on
set(gca, 'fontsize', 16)

% Subplot 2
subplot(2, 1, 2) % Subplot en la segunda posición
watermarked_signal=addWatermark(vect,500); 
[vectn]=featuresdd(watermarked_signal(:,1),watermarked_signal(:,2),vect(:,3));
plot(vectn(:,1),vectn(:,2),'k-')
title(['Watermarked con alpha ' num2str(500) ' MCYT user=414']);
grid on
set(gca, 'fontsize', 16)
...
% Subplot 3
subplot(2, 1, 1) % Subplot en la primera posición
watermarked_signal=addWatermark(vect,1000);
[vectn]=featuresdd(watermarked_signal(:,1),watermarked_signal(:,2),vect(:,3));
plot(vectn(:,1),vectn(:,2),'k-')
title(['Watermarked con alpha ' num2str(1000) ' MCYT user=414']) 
grid on
set(gca, 'fontsize', 16)

% Subplot 4
subplot(2, 1, 2) % Subplot en la segunda posición
watermarked_signal=addWatermark(vect,2000);
[vectn]=featuresdd(watermarked_signal(:,1),watermarked_signal(:,2),vect(:,3));
plot(vectn(:,1),vectn(:,2),'k-')
title(['Watermarked con alpha ' num2str(2000) ' MCYT user=414']) 
grid on
set(gca, 'fontsize', 16)
