%Script que permite comprobar las consecuencias del uso de un valor de
%alpha mas elevado en una marca de agua
...
for alpha=alphaset
    ...
    ResultadoString{index} = sprintf('Value of alpha=%s DTW Ident.(%%)=%s random forgeries DCF_opt=%s skilled DCF_opt=%s',num2str(alpha), num2str(tasa(point)), num2str(DCF_opt(point)), num2str(DCF_optS(point)));
    
    resultsT(index)=tasa(point);
    resultsOPT(index)=DCF_opt(point);
    resultsOPTS(index)=DCF_optS(point);
...
figure(1)
clf
plot(alphaset,resultsT)
xlabel('alpha')
ylabel('% de identificación')
grid on

figure(2)
clf
plot(alphaset,resultsOPT)
xlabel('alpha')
ylabel('DCF óptimo')
grid on
figure(2)
hold on
plot(alphaset,resultsOPTS)
legend('DCF opt random','DCF opt skilled')
